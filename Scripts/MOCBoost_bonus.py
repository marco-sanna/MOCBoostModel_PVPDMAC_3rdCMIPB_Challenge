...
def main():
    ...
    # bonus task data
    bonus = pd.read_csv("./data/bonus_task_data/Th1(IFN-γ)_Th2(IL-5)_polarization_ratio.tsv", sep ="\t")
    bonus["specimen_id"] = bonus["specimen_id"].astype(str)
    ...
    ## Merge with bonus task data
    merged_df = pd.merge(
        merged_df, 
            bonus[["specimen_id", "PT_P01579(IFN\u03b3)/PT_P05113(IL5)"]],  
        on="specimen_id", 
        how="left"  # Use 'left' to preserve all rows in merged_df
    )    
    ...
    #output_path = "./Train/merged_training_matrix_allmeta_wbonus.csv"
    ...
    outcomes_cols = train_matrix_woutcm_wide[["Monocytes_D1", "CCL3_D3", "IgG_PT_D14", "PT_P01579(IFNγ)/PT_P05113(IL5)_D30"]]
    ...
    output_list = ["Monocytes_D1",
                   ...
                   "IgG_PT_D14", 
                   "IgG_PT_FC",
                   "PT_P01579(IFN\u03b3)/PT_P05113(IL5)_D30"] #bonus task
    ...
    t_pol_cols = [col + "_D0" for col in t_pol_cols] + ["PT_P01579(IFN\u03b3)/PT_P05113(IL5)_D0", 
                                                        "PT_P01579(IFN\u03b3)/PT_P05113(IL5)_D30"]
    
    # List of covariates features
    ...
    #### 2) FEATURE SELECTION WITH HSIC LASSO
    ...
    # Retrieve feature names after calling input to get self.X_in (HSIC method) from the loaded data
    ...
    # Get selected features from HSIC LASSO
    ...
    baseline_feats = ["Monocytes_D0", "CCL3_D0", "IgG_PT_D0", "PT_P01579_D0", 
                      "PT_P05113_D0", "PT_P01579(IFN\u03b3)/PT_P05113(IL5)_D0", "infancy_vac", 
    ...
    def train_and_evaluate(X, y):
        ...
        plt.legend()
        plt.savefig(f"{output_dir}/optimized_MOCBoost_output_true_vs_pred.png")
        plt.close()
        ...
        # Initialize MultiOutputRegressor with tuned MultiOutputRegressor
        model = MultiOutputRegressor(
            CatBoostRegressor(
                **best_params,
                loss_function='RMSE',
    ...
    t_pol_cols_val = t_pol_cols
    t_pol_cols_val.remove("PT_P01579(IFN\u03b3)/PT_P05113(IL5)_D30")
